...
def run_dlc(config_path: Path, video_path: Path) -> pd.DataFrame:
    ...
    # output is <video_stem>DLC_*.h5 next to the video
    h5_matches = sorted(video_path.parent.glob(f"{video_path.stem}DLC*.h5"))
    if not h5_matches:
...
def process_dlc_tracking(op_dir: str, config: str, csv_path: Optional[str] = None):
    op_path = Path(op_dir)
    ...
    dlc_df = run_dlc(config_path, video_path)
...
def main():
    ...
    try:
        process_dlc_tracking(args.op_folder, args.config, args.csv)
